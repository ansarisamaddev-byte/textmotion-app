...
export default function TranscriptSidebar({ captions, activeId, onUpdate, onAdd, onDelete }) {
  return (
    <aside className="w-[380px] border-r border-zinc-800 bg-zinc-900/20 flex flex-col h-full">
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/40">
        <div className="flex items-center gap-2">
          <Type className="w-4 h-4 text-indigo-400" />
          <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-400">Captions</h2>
        </div>
        <button 
          onClick={onAdd}
          className="flex items-center gap-1.5 bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-600/20 px-2.5 py-1 rounded-md text-xs font-medium transition"
        >
        ...
        {captions.map((cap) => (
          ...
          >
            ...
            <textarea
              ...
              className="w-full bg-zinc-950/60 border border-zinc-800/80 rounded-lg p-2 text-xs text-zinc-200 focus:outline-none focus:border-indigo-500/50 resize-none h-14"
